...
- Utility `getScrollBehavior()` in `client/src/utils/scrollBehavior.ts` (see [Client Utils](./utils.md)) returns `'auto'` when `matchMedia('(prefers-reduced-motion: reduce)').matches`, otherwise `'smooth'`.
