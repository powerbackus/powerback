 ...
 * DockingManager
 *
 * Coordinates docking collection lifecycle for a given logical collection.
 * Provides helpers for creating docking collections, computing dry-run
 * promotion diffs, promoting staged documents to live collections, and
 * performing basic rollback and stats operations.
 */
class DockingManager {
  ...
  /**
   * Establishes a MongoDB connection for docking operations.
   *
   * Uses the shared database connector and caches the native db handle
   * on the instance for subsequent collection-level operations.
   *
   * Safe to call multiple times; it will only connect when needed.
   *
  ...
  /**
   * Ensures a docking collection exists for the given base collection.
   *
   * Naming convention is `docking_<collectionName>`. If the collection
   * already exists it is left untouched.
   *
   * @param {string} collectionName - Logical base collection name (e.g. 'pols').
   * @returns {Promise<string>} The docking collection name.
   */
  ...
  /**
   * Upserts a single document into a docking collection.
   *
   * Delegates to MongoDB's `updateOne` with `{ upsert: true }`, using
   * the provided filter (typically `{ id: <id> }`) to identify the doc.
   *
   * @param {string} collectionName - Logical base collection name.
   * @param {Object} data - Document payload to stage in docking.
   * @param {Object} filter - Filter used to match an existing docking doc.
   * @returns {Promise<import('mongodb').UpdateResult>} Raw MongoDB update result.
   */
   ...
   *
   * This method performs zero writes and is safe to run in any
   * environment where read access is permitted.
   *
   * @param {string} collectionName - Logical base collection name.
   * @returns {Promise<{
   *   inserts: Object[],
   *   updates: { id: string, before: Object, after: Object }[],
   *   unchanged: Object[]
   * }>} Buckets describing what promotion would do.
   * Zero writes performed.
   */
  ...
  /**
   * Promotes all documents from a docking collection into the live collection.
   *
   * For each staged doc it performs an upsert into the live collection,
   * keyed by `id`, and then removes the promoted docs from the docking
   * collection. This method assumes callers have already inspected
   * staged data (e.g. via stats/dryrun) before invoking it.
   *
   * @param {string} collectionName - Logical base collection name.
   * @returns {Promise<boolean>} True when promotion completes.
   */
  async promoteDockingCollection(collectionName) {
    ...
    for (const doc of docs) {
      const { _id, ...rest } = doc;
      await live.updateOne({ id: doc.id }, { $set: rest }, { upsert: true });
      count++;
    }
  ...
  /**
   * Rolls back a live collection to the most recent backup snapshot.
   *
   * Backups are expected to follow the `<collection>_backup_<timestamp>`
   * naming convention. The current live collection is dropped and the
   * newest backup is renamed into its place.
   *
   * @param {string} collectionName - Logical base collection name.
   * @returns {Promise<boolean>} True when rollback succeeds.
   * @throws {Error} When no backup collection exists.
   */
  ...
  /**
   * Returns basic stats for the live and docking collections.
   *
   * For each side it reports document count, total size, and average
   * object size. If a collection does not exist an `error` field is set.
   *
   * @param {string} collectionName - Logical base collection name.
   * @returns {Promise<{
   *   live: { count?: number, size?: number, avgObjSize?: number, error?: string },
   *   docking: { count?: number, size?: number, avgObjSize?: number, error?: string }
   * }>}
   */
  ...
  /**
   * Samples documents from the live and docking collections for comparison.
   *
   * Uses MongoDB `$sample` aggregation to pull up to `sampleSize` docs
   * from each side, which can then be diffed manually or logged.
   *
   * @param {string} collectionName - Logical base collection name.
   * @param {number} [sampleSize=10] - Number of docs to sample per collection.
   * @returns {Promise<{ live: Object[], docking: Object[] }>}
   */
...
/**
 * CLI entrypoint for DockingManager.
 *
 * Expected usage:
 *   node services/utils/dockingManager.js <command> <collection>
 *
 * Commands:
 *   - create:   ensure docking collection exists for <collection>
 *   - dryrun:   print promotion diff without writing
 *   - promote:  upsert docking docs into live and clear docking
 *   - rollback: restore live collection from most recent backup
 *   - stats:    show basic stats for live and docking collections
 *   - compare:  sample and print docs from live and docking
 *
 * Exits with code 1 on errors so it can be used in scripts.
 *
 * @returns {Promise<void>}
 */
async function main() {
  ...
  try {
    switch (command) {
      ...
      default:
        console.log(`
Usage: node services/utils/dockingManager.js <command> <collection>
