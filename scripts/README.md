...
- **pfp-sync.js** – Download House Clerk JPGs for live `pols` (House, `has_stakes`, not `roster_excluded`), write optimized `{bioguide}.webp` under `PFP_SYNC_OUT_DIR` (default `client/public/pfp`). Flags: `--dry-run`, `--force`, `--strict`. Intended for **VPS cron** (Mongo + `pfp` on same machine), not GitHub Actions deploy. See [Docking runbook – House headshot files](../docs/docking-pols-runbook.md#house-headshot-files-pfp-webp-sync). Run via `npm run pfp-sync`.
