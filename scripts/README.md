...
- **roster-exclude-pol.js** – Interactive TUI (`inquirer`: lists and confirms) to set or clear `Pol.roster_excluded` by bioguide ID. Optional first argument prefills bioguide; no `--category` / `--reason` flags. Loads `MONGODB_URI` from `.env.cli` / `.env.local` / `.env` **only after** you confirm—cancel exits without connecting. Details: [Pol roster exclusion spec](../specs/pol-roster-exclusion.md).
